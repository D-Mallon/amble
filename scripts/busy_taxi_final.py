...
for i in range(24):
    temp.append(float(weather_data['forecast']
                ['forecastday'][0]['hour'][i]['temp_f']))
    hum.append(float(weather_data['forecast']
               ['forecastday'][0]['hour'][i]["humidity"]))
    percip.append(float(weather_data['forecast']
                  ['forecastday'][0]['hour'][i]["precip_in"]))
...
selected_columns = ['Hour', 'Timestamp', 'Busyness Predicted', 'Holiday_False',
                    'Holiday_True', 'Month_7', 'Month_8', 'Day_Weekday', 'Day_Weekend']
